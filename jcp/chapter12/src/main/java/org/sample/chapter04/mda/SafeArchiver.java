...
public class SafeArchiver {
     ...
     * 执行分解任务的线程池
     */
    final private ThreadPoolExecutor forkPool = new ThreadPoolExecutor(
            3, 3,
            5, TimeUnit.SECONDS,
            new LinkedBlockingQueue<Runnable>(1000),
            Executors.defaultThreadFactory(), new ThreadPoolExecutor.AbortPolicy());
    ...
    public void archive(String name) {
        forkPool.submit(new LogicTask(name));
    }
    ...
    public static void main(String[] args) throws Exception {
        ...
        // 关闭执行分解任务的线程池
        archiver.forkPool.shutdown();
        while ( !archiver.forkPool.isTerminated() ) {
            Thread.sleep(1000);
        }
